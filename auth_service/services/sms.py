...
from utils.log_utils import get_struct_logger


def send_sms(phone, message):
    struct_logger = get_struct_logger()
    sms_obj = sms()
    struct_logger.info("send sms request", phone=phone, message=message)
    success = sms_obj.send(phone, message)
    struct_logger.info("send sms response",
                       phone=phone,
                       message=message,
                       success=success
                       )
...
class sms:
    ...
    def __init__(self):
        ...
        self.cred={'username': "9908835600", 'password': "Passw0rd"}
